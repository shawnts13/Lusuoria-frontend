      ...
      <a-table :columns="columns" :data-source="tableData" :loading="loading"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'progress'">
            <div style="display:flex;flex-direction:column;gap:2px;width:112px">
              ...
              <!-- 已实施：这个需求下实际关联了多少笔红人合作跟踪记录（establishedCount，不看进度
                   ...
                   条目名额已经全部建立了跟踪记录但还没有一条走到终态时，会显示 0/3（已实施3）） -->
              <span style="font-size:12px;color:#595959">（已实施：{{ record.establishedCount ?? 0 }}）</span>
            </div>
          </template>
...
const columns = [
  ...
  { title: '需求完成进度', key: 'progress', width: 160 },
